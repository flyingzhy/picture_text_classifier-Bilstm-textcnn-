...
from picturetextLSTM.cvtext_cnn_model import PictureTextCNN, PictureTextCNNConfig
from picturetextLSTM.pekingcontest.dataprocess import batch_iter, get_train, get_test, get_data, get_split_data
...
def train(x_train, y_train, x_val, y_val, num):
    save_dir = 'checkpoints/pekingtextcnn' + str(num)
    save_path = os.path.join(save_dir, 'best_validation')

    print("Configuring TensorBoard and Saver...")
    # 配置 Tensorboard，重新训练时，请将tensorboard文件夹删除，不然图会覆盖
    tensorboard_dir = 'tensorboard/pekingtextrnn'
...
if __name__ == "__main__":
    ...
    config = PictureTextCNNConfig()
    model = PictureTextCNN(config)
    ids = get_data(text_embedding_path)
    labels = get_data(labels_path)
